...
const TIER=[{bg:"#030e1f",acc:"#0a2248",clr:"#90caf9",gw:"#1565c0",lbl:"ICE"},{bg:"#021408",acc:"#083020",clr:"#80cbc4",gw:"#00695c",lbl:"JADE"},{bg:"#1c0f00",acc:"#402000",clr:"#ffe082",gw:"#f9a825",lbl:"GOLD"},{bg:"#180000",acc:"#3e0808",clr:"#ef9a9a",gw:"#c62828",lbl:"RUBY"}];
...
const mkRound=(sc,rnd,mode,settings,prevTr)=>{
  ...
  ],sc,rnd,cur:0,log:[`Round ${rnd} — Target: 21`],phase:mode==="bot"?"play":"pass",mode,settings};
};
...
const applyT=(gs,pid,tid)=>{
  ...
  if(nm){const val=+nm[1],di=s.deck.indexOf(val);if(di!==-1){s.deck.splice(di,1);me.h.push({v:val,fd:false});msg+=` → Drew ${val} (${hSum(me.h)})`;}else msg+=` → Not in deck`;}
  else switch(tid){
...
const checkEnd=s=>{
  ...
  else{const d0=Math.abs(T-s0),d1=Math.abs(T-s1);if(d0<d1){w=0;msg=`${s.mode==="bot"?"You":"P1"} win! ${s0} vs ${s1}`;}else if(d1<d0){w=1;msg=`${s.mode==="bot"?"Bot":"P2"} wins! ${s1} vs ${s0}`;}else{w=-1;msg=`Draw! Both ${s0}`;}}
...
const GameCard=({v,fd,idx=0,small=false,justFlipped=false})=>{
  const t=v?TIER[cTier(v)]:TIER[0];const W=small?44:58,H=small?64:84;
  return(<div style={{width:W,height:H,flexShrink:0,animation:justFlipped?"cardFlip .55s ease both":`cIn .3s ease ${idx*55}ms both`}}>
    <div style={{width:"100%",height:"100%",borderRadius:7,transform:"perspective(200px) rotateX(4deg)",boxShadow:fd?"0 6px 18px rgba(0,0,0,0.8)":`0 6px 18px rgba(0,0,0,0.7),0 0 14px ${t.gw}55`}}>
      {fd?(<div style={{width:"100%",height:"100%",borderRadius:7,background:"linear-gradient(145deg,#400000,#720808)",border:"1.5px solid rgba(200,160,80,0.45)",position:"relative",overflow:"hidden"}}>
      ...
      </div>):(<div style={{width:"100%",height:"100%",borderRadius:7,background:`linear-gradient(145deg,${t.acc},${t.bg})`,border:`1.5px solid ${t.gw}44`,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:3,left:4}}><div style={{fontSize:small?9:11,fontWeight:900,color:t.clr,lineHeight:1,textShadow:`0 0 8px ${t.gw}`}}>{v}</div><div style={{fontSize:5,color:t.gw,lineHeight:1}}>◆</div></div>
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:small?22:30,fontWeight:900,color:t.clr,textShadow:`0 0 18px ${t.gw},0 0 6px ${t.gw}88`}}>{v}</span></div>
        ...
        <div style={{position:"absolute",bottom:3,right:4,transform:"rotate(180deg)"}}><div style={{fontSize:small?9:11,fontWeight:900,color:t.clr,lineHeight:1}}>{v}</div><div style={{fontSize:5,color:t.gw,lineHeight:1}}>◆</div></div>
      </div>)}
    </div>
  </div>);
};
...
const TrumpCard=({tid,onClick,disabled,size=62})=>{
  const t=TD[tid];const[tip,setTip]=useState(false);const[tipDir,setTipDir]=useState("center");
  const hRef=useRef(null);const cRef=useRef(null);const H=Math.round(size*1.42);
  if(!t)return null;
  const enter=()=>{
    if(cRef.current){const r=cRef.current.getBoundingClientRect(),tw=200;
      if(r.left<tw/2+10)setTipDir("right");else if(window.innerWidth-r.right<tw/2+10)setTipDir("left");else setTipDir("center");}
  ...
  return(<div ref={cRef} style={{position:"relative",width:size,height:H,flexShrink:0}} onMouseEnter={enter} onMouseLeave={leave}>
    <button onClick={onClick} disabled={disabled} style={{width:"100%",height:"100%",padding:0,border:"none",background:"none",cursor:disabled?"not-allowed":"pointer",borderRadius:8,transform:disabled?"none":"perspective(200px) rotateX(4deg)",boxShadow:disabled?"none":`0 5px 14px rgba(0,0,0,0.6),0 0 10px ${t.glow}44,0 4px 0 ${t.color}aa`,transition:"transform 0.15s,box-shadow 0.15s"}}
      ...
      <div style={{width:"100%",height:"100%",borderRadius:8,background:`linear-gradient(145deg,${t.color}ee,${t.color}88)`,border:`1.5px solid ${t.glow}66`,position:"relative",overflow:"hidden",opacity:disabled?0.25:1}}>
        ...
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:size>50?28:20,color:t.glow,textShadow:`0 0 14px ${t.glow}`,lineHeight:1}}>{t.sym}</div></div>
    ...
    {tip&&!disabled&&(<div style={{position:"absolute",bottom:"110%",zIndex:999,width:200,background:"rgba(4,2,0,0.97)",border:`1px solid ${t.glow}55`,borderRadius:10,padding:"10px 12px",boxShadow:`0 8px 28px rgba(0,0,0,0.85),0 0 14px ${t.glow}44`,pointerEvents:"none",...tipPos}}>
...
const TableTrump=({tid,active=false})=>{const t=TD[tid];if(!t)return null;return(<div style={{width:44,height:62,flexShrink:0,borderRadius:7,background:`linear-gradient(145deg,${t.color}cc,${t.color}66)`,border:`1.5px solid ${active?t.glow+"88":t.glow+"33"}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,boxShadow:active?`0 3px 10px rgba(0,0,0,0.5),0 0 10px ${t.glow}55`:"0 2px 6px rgba(0,0,0,0.4)",opacity:active?1:0.6,position:"relative"}}><div style={{fontSize:18,color:t.glow,textShadow:active?`0 0 12px ${t.glow}`:"none",lineHeight:1}}>{t.sym}</div><div style={{fontSize:5.5,color:`${t.glow}cc`,fontFamily:"Georgia,serif",textTransform:"uppercase",letterSpacing:0.5,textAlign:"center",lineHeight:1.2,padding:"0 3px"}}>{t.n}</div>{active&&<div style={{position:"absolute",top:-3,right:-3,width:7,height:7,borderRadius:"50%",background:t.glow,boxShadow:`0 0 6px ${t.glow}`}}/>}</div>);};

const Chip=({n,color,label})=>(<div style={{textAlign:"center"}}><div style={{width:38,height:38,borderRadius:"50%",margin:"0 auto",background:`radial-gradient(circle at 35% 30%,${color}ee,${color}88)`,border:"3px solid rgba(255,255,255,0.18)",boxShadow:`0 4px 10px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.2)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,fontWeight:900,color:"#fff",textShadow:"0 1px 3px rgba(0,0,0,0.8)"}}>{n}</div>{label&&<div style={{fontSize:8,color:"#5a4a30",marginTop:4,letterSpacing:0.8,textTransform:"uppercase"}}>{label}</div>}</div>);

const Btn3D=({label,onClick,disabled,top,bot,shad,sx={}})=>{const[pr,setPr]=useState(false);return(<button onMouseDown={()=>setPr(true)} onMouseUp={()=>setPr(false)} onMouseLeave={()=>setPr(false)} onClick={onClick} disabled={disabled} style={{padding:"11px 28px",fontSize:14,fontWeight:900,borderRadius:11,border:"none",cursor:disabled?"not-allowed":"pointer",background:disabled?"rgba(8,6,2,0.6)":pr?bot:`linear-gradient(180deg,${top},${bot})`,color:disabled?"#1e1a10":"#fff",boxShadow:disabled?"none":pr?`0 1px 0 ${shad}`:`0 5px 0 ${shad},0 7px 14px rgba(0,0,0,0.5)`,transform:pr?"translateY(4px)":"none",transition:"all 0.09s",letterSpacing:"0.5px",textShadow:disabled?"none":"0 1px 2px rgba(0,0,0,0.5)",fontFamily:"Georgia,serif",...sx}}>{label}</button>);};

const LogPanel=({log})=>{const el=useRef(null);useEffect(()=>{if(el.current)el.current.scrollTop=el.current.scrollHeight;},[log]);return(<div ref={el} style={{height:"100%",overflowY:"auto",padding:"6px 8px"}}>{log.map((l,i)=><div key={i} style={{fontSize:10.5,color:i===log.length-1?"#d4c078":"#3a3020",padding:"2px 0",lineHeight:1.4,borderBottom:"1px solid rgba(255,255,255,0.02)"}}>{l}</div>)}</div>);};
...
const PlayerRow=({player,pid,isMe,isBot,mode,T,flippedSet})=>{
  ...
  const label=mode==="bot"?(pid===0?"You":"Bot"):(pid===0?"Player 1":"Player 2");
  return(<div style={{display:"flex",alignItems:"center",gap:16,width:"100%"}}>
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,flexShrink:0,width:64}}>
      <div style={{width:44,height:44,borderRadius:"50%",background:pid===0?"#0d2a4a":"#4a0808",border:`2px solid ${pColor}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:pid===1&&isBot?20:14,fontWeight:900,color:pColor,boxShadow:`0 2px 8px ${pColor}33`}}>{pid===1&&isBot?"🤖":"P"+(pid+1)}</div>
    ...
    <div style={{display:"flex",gap:6,alignItems:"center",flex:1,flexWrap:"wrap"}}>
      {player.h.map((c,i)=><GameCard key={i} v={c.v} fd={c.fd} idx={i} justFlipped={!!flippedSet&&flippedSet.has(`${pid}-${i}`)}/>)}
    </div>
    <div style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
      <div style={{minWidth:56,height:46,borderRadius:10,background:isMe?(bust?"rgba(100,0,0,0.7)":s===T?"rgba(0,90,35,0.7)":"rgba(5,3,1,0.7)"):"rgba(5,3,1,0.7)",border:`1px solid ${isMe?(bust?"#b02020":s===T?"#2e7d32":"rgba(212,192,120,0.12)"):"rgba(212,192,120,0.12)"}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",boxShadow:isMe&&s===T?"0 0 14px rgba(0,180,70,0.3)":isMe&&bust?"0 0 12px rgba(190,0,0,0.3)":"none"}}>
        {hiddenCount>0?<div style={{fontSize:12,fontWeight:900,color:"#d4c078",textAlign:"center",padding:"0 4px"}}>?+{visSum}</div>
...
/* ─── ROUND END POPUP ────────────────────────────────────────────────── */
const RoundPopup=({gs,T,isBot,onNext,onHome})=>{
  const wl=gs.winner===-1?"Draw":gs.winner===0?(isBot?"You Win!":"P1 Wins!"):(isBot?"Bot Wins!":"P2 Wins!");
  const wc=gs.winner===0?"#7ec8e3":gs.winner===-1?"#d4c078":"#f0a080";
  const maxR=gs.settings?.maxRounds||5,need=winThreshold(maxR);
  return(<div style={{position:"fixed",inset:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.75)"}}>
    <div style={{background:"radial-gradient(ellipse at 50% 20%,#1a3020 0%,#080e0a 100%)",border:"1.5px solid rgba(212,192,120,0.3)",borderRadius:20,padding:"32px 40px",minWidth:420,maxWidth:520,boxShadow:"0 24px 60px rgba(0,0,0,0.9)",animation:"popIn .35s cubic-bezier(0.34,1.56,0.64,1) both",textAlign:"center"}}>
      ...
      <div style={{fontSize:28,fontWeight:900,color:wc,letterSpacing:2,marginBottom:4}}>{wl}</div>
      <div style={{fontSize:10,color:"#4a4030",letterSpacing:2,marginBottom:18,textTransform:"uppercase"}}>Round {gs.rnd} · First to {need} wins · {gs.settings?.maxRounds||5}-round match</div>
      <div style={{display:"flex",gap:32,justifyContent:"center",marginBottom:18}}>
        {gs.p.map((p,i)=>{const s=hSum(p.h),b=s>T;return(<div key={i} style={{textAlign:"center"}}>
          <div style={{color:i===0?"#7ec8e3":"#f0a080",fontWeight:700,marginBottom:7,fontSize:11,letterSpacing:1,textTransform:"uppercase"}}>{isBot?(i===0?"You":"Bot"):`P${i+1}`}</div>
      ...
      <div style={{display:"flex",gap:20,justifyContent:"center",marginBottom:22}}>
        <Chip n={gs.sc[0]} color="#1565c0" label={isBot?"You":"P1"}/>
        <div style={{display:"flex",flexDirection:"column",justifyContent:"center",gap:2,textAlign:"center"}}>
          ...
          <div style={{fontSize:22,fontWeight:900,color:"#6a5820"}}>{gs.sc[0]} — {gs.sc[1]}</div>
        </div>
        <Chip n={gs.sc[1]} color="#6a1b1b" label={isBot?"Bot":"P2"}/>
      </div>
...
const SettingsModal=({settings,onChange,onClose})=>{
  ...
  const opts=(vals,cur,set,label,fmt)=>(<div style={{marginBottom:16}}>
    <div style={{fontSize:10,letterSpacing:2,color:"#5a4a30",textTransform:"uppercase",marginBottom:8}}>{label}</div>
    <div style={{display:"flex",gap:8}}>
      {vals.map(v=><button key={v} onClick={()=>set(v)} style={{flex:1,padding:"10px 0",borderRadius:10,border:`1px solid ${cur===v?"rgba(212,192,120,0.6)":"rgba(212,192,120,0.15)"}`,background:cur===v?"rgba(212,192,64,0.12)":"rgba(0,0,0,0.3)",color:cur===v?"#d4c078":"#5a4a30",cursor:"pointer",fontSize:13,fontWeight:cur===v?900:400,fontFamily:"Georgia,serif",transition:"all 0.15s"}}>{fmt(v)}</button>)}
    </div>
  </div>);
  return(<div style={{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.8)"}}>
...
/* ─── CREATE ROOM SCREEN ─────────────────────────────────────────────── */
const CreateRoomScreen=({settings,onBack,roomCodeRef,mkRound,setGs,setRoomCode,setMyPid})=>{
...
const JoinRoomScreen=({onBack,onJoin})=>{
  ...
  return(<div style={{...FELT,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",gap:24}}>
    ...
    <div style={{background:"rgba(0,0,0,0.5)",border:"1.5px solid rgba(212,192,120,0.25)",borderRadius:16,padding:"28px 36px",textAlign:"center",boxShadow:"0 12px 40px rgba(0,0,0,0.6)",minWidth:320}}>
      ...
      <Btn3D label="Join Room" onClick={submit} top="#d4a820" bot="#9a7010" shad="#5a4000" sx={{width:"100%",padding:"12px 0"}}/>
    </div>
    <div style={{background:"rgba(0,0,0,0.35)",borderRadius:12,padding:"12px 18px",border:"1px solid rgba(212,192,120,0.1)",maxWidth:340,textAlign:"center"}}>
      <div style={{fontSize:11,color:"#5a4820",lineHeight:1.7}}>Ask your friend for their <b style={{color:"#8a7040"}}>room code</b> and enter it above. Once deployed with a real-time backend, this will connect you to their game instantly.</div>
    </div>
    <Btn3D label="← Back" onClick={onBack} top="#3a3020" bot="#1a1810" shad="#0a0804"/>
  </div>);
};
  ...
  useEffect(()=>{
    ...
    flips.forEach(({pid,ci,key},i)=>{
      const t1=setTimeout(()=>{
        ...
        setGs(prev=>{if(!prev||prev.phase!=="reveal")return prev;const ns=JSON.parse(JSON.stringify(prev));ns.p[pid].h[ci].fd=false;return ns;});
      },500+i*500);
    ...
    const tF=setTimeout(()=>{setGs(prev=>{if(!prev||prev.phase!=="reveal")return prev;const ns=JSON.parse(JSON.stringify(prev));ns.phase=ns.matchOver?"matchEnd":"roundEnd";return ns;});},500+flips.length*500+700);
    revTimers.current.push(tF);
    return()=>revTimers.current.forEach(clearTimeout);
  },[gs?.phase==="reveal"]);

  const startGame=mode=>{
    // HOST: detect when player 2 joins
useEffect(()=>{
...
// BOTH PLAYERS: sync opponent moves during game
useEffect(()=>{
...
useEffect(()=>{
  ...
  const advanceAfter=(ns,actingPid)=>{
    ...
    if(nx.mode==="pvp"){nx.cur=1-actingPid;nx.phase="pass";}else nx.cur=1-actingPid;
  ...
  if(!gs||gs.phase!=="play"||gs.cur!==activePid||gs.p[activePid].std)return;
  const ns=JSON.parse(JSON.stringify(gs));
  ns.p[activePid].std=true;ns.log.push(`P${activePid+1} stands at ${hSum(ns.p[activePid].h)}.`);
  const ck=checkEnd(ns);if(ck.phase!=="play"){setFlippedSet(new Set());setGs(ck);return;}
  const nx=JSON.parse(JSON.stringify(ck));nx.cur=1-activePid;setGs(nx);
},[gs,activePid]);


  const nextRound=()=>{
    if(!gs)return;
    setGs(mkRound(gs.sc,gs.rnd<gs.settings.maxRounds?gs.rnd+1:gs.rnd,gs.mode,gs.settings,[gs.p[0].tr,gs.p[1].tr]));
  ...
  const onTimerExpire=useCallback(()=>{
    setGs(prev=>{
      if(!prev||prev.phase!=="play"||prev.cur!==0||prev.p[0].std)return prev;
      const ns=JSON.parse(JSON.stringify(prev));
      ns.p[0].std=true;ns.log.push("⏰ Time's up! You auto-stand.");
  ...
  /* ── HOME ── */
  if(screen==="home"){
    return(<div style={{...FELT,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",gap:36}}>
      <style>{GFX}</style>
      ...
      <div style={{display:"flex",gap:16,flexWrap:"wrap",justifyContent:"center",animation:"slideIn 0.7s ease 0.2s both"}}>
        {/* vs Bot */}
        <button onClick={()=>startGame("bot")} style={{width:160,padding:"22px 16px",borderRadius:16,border:"1.5px solid rgba(212,192,120,0.25)",background:"rgba(0,0,0,0.4)",cursor:"pointer",textAlign:"center",transition:"all 0.2s",boxShadow:"0 6px 24px rgba(0,0,0,0.5)"}}
          ...
          <div style={{fontSize:17,fontWeight:900,color:"#d4c078",marginBottom:3}}>vs Bot</div>
          <div style={{fontSize:10,color:"#6a5820",letterSpacing:1,textTransform:"uppercase"}}>Solo Play</div>
        </button>
        {/* Create Room */}
        <button onClick={()=>setScreen("create")} style={{width:160,padding:"22px 16px",borderRadius:16,border:"1.5px solid rgba(212,192,120,0.25)",background:"rgba(0,0,0,0.4)",cursor:"pointer",textAlign:"center",transition:"all 0.2s",boxShadow:"0 6px 24px rgba(0,0,0,0.5)"}}
          onMouseEnter={e=>{e.currentTarget.style.background="rgba(30,20,5,0.7)";e.currentTarget.style.borderColor="rgba(212,192,120,0.6)";e.currentTarget.style.transform="translateY(-4px)";}}
          onMouseLeave={e=>{e.currentTarget.style.background="rgba(0,0,0,0.4)";e.currentTarget.style.borderColor="rgba(212,192,120,0.25)";e.currentTarget.style.transform="none";}}>
          <div style={{fontSize:32,marginBottom:8}}>🏠</div>
          <div style={{fontSize:17,fontWeight:900,color:"#d4c078",marginBottom:3}}>Create Room</div>
          <div style={{fontSize:10,color:"#6a5820",letterSpacing:1,textTransform:"uppercase"}}>1 vs 1 · Host</div>
        </button>
        {/* Find Room */}
        <button onClick={()=>setScreen("join")} style={{width:160,padding:"22px 16px",borderRadius:16,border:"1.5px solid rgba(212,192,120,0.25)",background:"rgba(0,0,0,0.4)",cursor:"pointer",textAlign:"center",transition:"all 0.2s",boxShadow:"0 6px 24px rgba(0,0,0,0.5)"}}
          onMouseEnter={e=>{e.currentTarget.style.background="rgba(30,20,5,0.7)";e.currentTarget.style.borderColor="rgba(212,192,120,0.6)";e.currentTarget.style.transform="translateY(-4px)";}}
          onMouseLeave={e=>{e.currentTarget.style.background="rgba(0,0,0,0.4)";e.currentTarget.style.borderColor="rgba(212,192,120,0.25)";e.currentTarget.style.transform="none";}}>
          <div style={{fontSize:32,marginBottom:8}}>🔍</div>
          <div style={{fontSize:17,fontWeight:900,color:"#d4c078",marginBottom:3}}>Find Room</div>
          <div style={{fontSize:10,color:"#6a5820",letterSpacing:1,textTransform:"uppercase"}}>1 vs 1 · Join</div>
        </button>
      </div>
...
// Use myPid so each device sees their own perspective
const activePid=isBot?0:myPid;
const myTurn=gs.phase==="play"&&gs.cur===activePid&&!gs.p[activePid].std;
...
const op=gs.p[1-activePid];
  ...
  /* PASS */
  if(gs.phase==="pass"){
    const next=gs.cur===0?"Player 1":"Player 2",col=gs.cur===0?"#7ec8e3":"#f0a080";
    return(<div style={{...FELT,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",gap:28}}>
      <style>{GFX}</style>
      <div style={{fontSize:56,animation:"spin 4s linear infinite"}}>🃏</div>
      <div style={{textAlign:"center"}}>
        <div style={{fontSize:22,fontWeight:900,color:"#d4c078",marginBottom:6}}>Pass the device</div>
        <div style={{fontSize:15,color:"#8a7040"}}>Hand it to <b style={{color:col}}>{next}</b></div>
        <div style={{fontSize:12,color:"#3a3020",marginTop:4}}>Don't peek 🙈</div>
      </div>
      <div style={{display:"flex",gap:20}}><Chip n={gs.sc[0]} color="#1565c0" label="P1"/><Chip n={gs.sc[1]} color="#6a1b1b" label="P2"/></div>
      <Btn3D label={`I'm ${next} — Ready`} onClick={()=>{setGs(s=>({...s,phase:"play"}));setTimerKey(k=>k+1);}} top="#d4a820" bot="#9a7010" shad="#5a4000"/>
    </div>);
  }

  /* MATCH END */
  if(gs.phase==="matchEnd"){
    const mw=gs.sc[0]>=winThreshold(gs.settings.maxRounds)?(isBot?"You":"Player 1"):(isBot?"Bot":"Player 2");
    return(<div style={{...FELT,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",gap:20}}>
      <style>{GFX}</style>
      ...
      <div style={{display:"flex",gap:16}}><Chip n={gs.sc[0]} color="#1565c0" label={isBot?"You":"P1"}/><Chip n={gs.sc[1]} color="#6a1b1b" label={isBot?"Bot":"P2"}/></div>
  ...
  /* PLAY + REVEAL + POPUP */
  return(<div style={{...FELT,minHeight:"100vh",display:"flex",gap:0,position:"relative"}}>
    <style>{GFX}</style>
    {isRevealing&&<div style={{position:"fixed",inset:0,zIndex:40,pointerEvents:"none"}}>
    ...
    {gs.phase==="roundEnd"&&<RoundPopup gs={gs} T={T} isBot={isBot}
      onNext={gs.rnd<gs.settings.maxRounds?nextRound:newMatch}
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",padding:"16px 20px 16px 16px",gap:0,minWidth:0}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,background:"rgba(0,0,0,0.38)",borderRadius:12,padding:"10px 16px",border:"1px solid rgba(212,192,120,0.08)"}}>
        <div style={{display:"flex",gap:24,alignItems:"center"}}>
          <div><div style={{fontSize:8,letterSpacing:3,color:"#3a3020",textTransform:"uppercase"}}>Round</div><div style={{fontSize:20,fontWeight:900,color:"#d4c078",lineHeight:1}}>{gs.rnd}<span style={{fontSize:10,color:"#3a3020"}}>/{gs.settings.maxRounds}</span></div></div>
          <div><div style={{fontSize:8,letterSpacing:3,color:"#3a3020",textTransform:"uppercase"}}>Target</div><div style={{fontSize:24,fontWeight:900,color:"#d4c078",lineHeight:1}}>{T}</div></div>
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            <Chip n={gs.sc[0]} color="#1565c0" label={isBot?"You":"P1"}/>
            <div style={{fontSize:9,color:"#2a2015",textAlign:"center",lineHeight:1.3}}>First to<br/><b style={{color:"#5a4820"}}>{winThreshold(gs.settings.maxRounds)}</b></div>
            <Chip n={gs.sc[1]} color="#6a1b1b" label={isBot?"Bot":"P2"}/>
          </div>
          {/* Timer */}
          {myTurn&&!isRevealing&&(
            <CountdownTimer key={timerKey} seconds={gs.settings.timerSec} active={myTurn} onExpire={onTimerExpire}/>
          )}
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
      ...
      <div style={{background:"rgba(0,0,0,0.2)",borderRadius:14,padding:"14px 16px",border:"1px solid rgba(212,192,120,0.06)",boxShadow:isRevealing?"0 0 18px rgba(212,192,64,0.08)":"none",transition:"box-shadow 0.4s"}}>
        ...
        {botThinking&&<div style={{display:"flex",gap:5,alignItems:"center",marginTop:10}}>{[0,1,2].map(i=><div key={i} style={{width:6,height:6,borderRadius:"50%",background:"#c08040",animation:`pulse 1s ease ${i*0.2}s infinite`}}/>)}<span style={{fontSize:11,color:"#4a3820",marginLeft:4,fontStyle:"italic"}}>Bot is thinking...</span></div>}
        {p1.std&&<div style={{marginTop:8,fontSize:10,color:"#4a8840"}}>● Standing</div>}
      </div>
      ...
      <div style={{margin:"10px 0",background:"rgba(0,0,0,0.15)",borderRadius:12,border:"1px solid rgba(212,192,120,0.07)",padding:"10px 16px",display:"flex",flexDirection:"column",gap:8}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",minHeight:66}}>
          <div style={{fontSize:8,letterSpacing:1.5,color:"#2a2015",textTransform:"uppercase",width:56,flexShrink:0,textAlign:"right",paddingRight:10,lineHeight:1.4}}>{isBot?"Bot":"P2"}<br/>played</div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap",flex:1,alignItems:"center"}}>
            {p1.played.length>0?p1.played.map((t,i)=><TableTrump key={i} tid={t} active={p1.tbl.includes(t)}/>):<div style={{fontSize:9,color:"#1a1408",fontStyle:"italic"}}>no trump cards played yet</div>}
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",minHeight:66}}>
          <div style={{fontSize:8,letterSpacing:1.5,color:"#2a2015",textTransform:"uppercase",width:56,flexShrink:0,textAlign:"right",paddingRight:10,lineHeight:1.4}}>{isBot?"You":"P1"}<br/>played</div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap",flex:1,alignItems:"center"}}>
            {p0.played.length>0?p0.played.map((t,i)=><TableTrump key={i} tid={t} active={p0.tbl.includes(t)}/>):<div style={{fontSize:9,color:"#1a1408",fontStyle:"italic"}}>no trump cards played yet</div>}
          </div>
        </div>
        <div style={{height:1,background:GL,borderRadius:1}}/>
      </div>
      ...
      <div style={{background:"rgba(0,0,0,0.28)",borderRadius:14,padding:"14px 16px",border:`1px solid ${isRevealing?"rgba(212,192,64,0.18)":myTurn?"rgba(212,192,64,0.25)":"rgba(212,192,120,0.06)"}`,boxShadow:myTurn?"0 0 20px rgba(212,192,64,0.06)":"none",transition:"border-color 0.3s"}}>
        ...
        <div style={{marginTop:12,paddingTop:10,borderTop:"1px solid rgba(212,192,120,0.07)"}}>
          <div style={{fontSize:8,letterSpacing:2,color:"#2a2015",textTransform:"uppercase",marginBottom:8}}>Your Trump Cards ({p0.tr.length}){p0.tr.length>0?" — hover 1s for details":""}</div>
          <div style={{display:"flex",gap:7,flexWrap:"wrap",minHeight:88,alignItems:"center"}}>
            {p0.tr.length>0?p0.tr.map((tid,i)=><TrumpCard key={i} tid={tid} disabled={p0.blk||p0.std||isRevealing} onClick={()=>doTrump(tid)}/>)
              :<div style={{fontSize:10,color:"#1a1408",fontStyle:"italic"}}>No trump cards in hand</div>}
          </div>
        </div>
        {myTurn&&hSum(p0.h)>T&&<div style={{marginTop:10,padding:"6px 12px",borderRadius:8,background:"rgba(80,0,0,0.4)",border:"1px solid rgba(180,40,40,0.3)",fontSize:11,color:"#ff8080",lineHeight:1.5}}>⚠ Over {T}! Use a trump card to recover, or Stand to lock in your bust.</div>}
        <div style={{display:"flex",gap:10,marginTop:12}}>
          {isRevealing?(<div style={{padding:"10px 16px",borderRadius:10,background:"rgba(0,0,0,0.2)",border:"1px solid rgba(212,192,120,0.1)",fontSize:12,color:"#6a5820",fontStyle:"italic",letterSpacing:1}}>Revealing all hidden cards...</div>)
            :myTurn?(<><Btn3D label="DRAW" onClick={doDraw} disabled={gs.deck.length===0} top="#1a4a7a" bot="#0c2a4a" shad="#050f20"/><Btn3D label="STAND" onClick={doStand} top="#1a5224" bot="#0c2e12" shad="#060f08"/></>)
            :(<div style={{padding:"10px 16px",borderRadius:10,background:"rgba(0,0,0,0.2)",border:"1px solid rgba(212,192,120,0.05)",fontSize:12,color:"#1e1a10",fontStyle:"italic"}}>{p0.std?"You are standing — waiting for opponent...":botThinking?"Bot is taking its turn...":"Waiting for your turn..."}</div>)}
        </div>
      </div>
    </div>

    {showLog&&<div style={{width:230,flexShrink:0,background:"rgba(0,0,0,0.45)",borderLeft:"1px solid rgba(212,192,120,0.07)",padding:"16px 0",display:"flex",flexDirection:"column"}}>
